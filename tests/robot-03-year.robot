...
TC1701 - เจ้าหน้าที่ฝ่ายทะเบียนเพิ่มปีการศึกษาสำเร็จ
...
TC1702 - เจ้าหน้าที่ฝ่ายทะเบียนเพิ่มปีการศึกษาไม่สำเร็จ(กดปุ่มบวกซ้ำอีกรอบ)
    [Documentation]    ทดสอบการเพิ่มปีการศึกษาที่มีอยู่แล้ว
    # เปิดเบราว์เซอร์ที่ URL ที่กำหนด โดยใช้ browser ที่ระบุ
    Open Browser    ${URL}    ${BROWSER}
    # ขยายหน้าต่างเบราว์เซอร์ให้เต็มจอ เพื่อป้องกันปัญหา element ไม่แสดง
    Maximize Browser Window
    ...
    Capture Page Screenshot With Name    TC1702_ExistingYear_Error
    Close Browser

TC1703 - เจ้าหน้าที่ฝ่ายทะเบียนแก้ไขปีการศึกษาสำเร็จ
    [Documentation]    ทดสอบการแก้ไขปีการศึกษาที่มีอยู่แล้ว
    # เปิดเบราว์เซอร์ที่ URL ที่กำหนด โดยใช้ browser ที่ระบุ
    Open Browser    ${URL}    ${BROWSER}
    # ขยายหน้าต่างเบราว์เซอร์ให้เต็มจอ เพื่อป้องกันปัญหา element ไม่แสดง
    Maximize Browser Window
    ...
    Sleep  5s
    # --- กดปุ่ม 3 จุด ---
    Click Element    xpath=//*[@id="dropdown_68e4159cad3930f4731e6b7f"]
    # --- กดปุ่ม แก้ไข ---
    Click Element    xpath=//*[@id="edit_year_68e4159cad3930f4731e6b7f"]
    # --- แก้ไขปีการศึกษา ---
    Edit Academic Year    
...
TC1705 - เจ้าหน้าที่ฝ่ายทะเบียนลบปีการศึกษาสำเร็จ
    ...
    Click Element    xpath=//*[@id="dropdown_68e4159cad3930f4731e6b7f"]
    # --- กดปุ่มลบ ---
    Click Element    xpath=//*[@id="delete_year_68e4159cad3930f4731e6b7f"]
    # --- กดปุ่มยืนยันการลบ ---
    Delete Academic Year Confirm

    Capture Page Screenshot With Name    TC1705_DeleteAcademicYear_Success
    Close Browser
...
Add Academic Year Error
    Click Element    xpath=//*[@id="add_year_button"]
    Wait Until Page Contains   สร้างปีการศึกษา 2571 ไม่สำเร็จ   timeout=15s
...
Edit Academic Year
    ...
    Sleep                   0.3s
    Click Button            xpath=//*[@id="Edit_year_68e4159cad3930f4731e6b7f"]/div/form/div[2]/button[1]
